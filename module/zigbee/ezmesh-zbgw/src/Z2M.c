...
void Logger_Z2M_Print(
    char *action, unsigned char *da,
    unsigned short len) { // AT+ 22 92 00 12 4B 00 05 A7 B6 C2 50 28 00 00 00 13
                          // 00 00 00 00 28 50 C2 B6 A7 05 00 4B 12 00 00 86
    ...
    if (!memcmp(&ED[ED_Index].MacAddress[0], &da[2], 8) && updated == 0) {
      if (updated == 0) {
        ...
        printf("(Z2M_AN)Update ED MacAddress = %s SA=%04X\n",
               Transfer_End_Device_Mac(ED[ED_Index].MacAddress),
               ED[ED_Index].ShortAddress);
        updated = 1;
      } else {
...
void Z2M_SD(uint8_t *da, uint16_t len) {
  ...
  printf(
      "Simple desc : Addr %02X, Endpoint %02X, Profile %04X, DeviceID %04X\r\n",
