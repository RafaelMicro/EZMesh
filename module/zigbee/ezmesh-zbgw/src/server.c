...
void tcp_client_list_insert(tcp_server_thread_t **list, tcp_server_thread_t *p)
{
...
tcp_server_thread_t *tcp_client_list_delete(tcp_server_thread_t **list, int sock)
{
...
tcp_server_thread_t *tcp_client_list_search(tcp_server_thread_t *list, int sock)
{
	...
	while (p)
	{
		...
		{		
			...
			printf("Send to %s: %d, len: %d\r\n", 
				inet_ntoa(p->client_addr.sin_addr), htons(p->client_addr.sin_port), len);
		}
		p = p->next;
	}
}
