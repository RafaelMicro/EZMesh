...
char EndDevice_Filename[] =
    "/usr/local/var/lib/ezmesh-zbgw/zbdb/sc_enddevice.dat";
char Coordinator_Filename[] =
    "/usr/local/var/lib/ezmesh-zbgw/zbdb/sc_coordinator.dat";
...
void Read_EndDevice_File() {
  ...
  if (FileExist(EndDevice_Filename)) {
    ...
    if (fp != NULL) {
      while (1) {
        rsize = fread(&ED[CR.DevCount].MacAddress[0], sizeof(struct _EndDevice),
                      1, fp);
        if (rsize != 1)
          break;
        //-------------------------------------------------------------------------
...
char *Transfer_End_Device_Mac(unsigned char *src_mac) {
  ...
  snprintf((char *)Transfer_Device_ED_dest_mac, 20,
           "%02X%02X%02X%02X%02X%02X%02X%02X", src_mac[0], src_mac[1],
           src_mac[2], src_mac[3], src_mac[4], src_mac[5], src_mac[6],
           src_mac[7]);
...
void show_dev_info() {
  int i, j, k;
  // printf("|      ExtAddr     | Short Addr | Device ID | EP List | Cluster
  // (In) | \r\n");
  ...
  for (i = 0; i < EndDeviceMax; i++) {
    ...
    printf("\t MAC Address :%02X%02X%02X%02X%02X%02X%02X%02X \r\n",
           ED[i].MacAddress[0], ED[i].MacAddress[1], ED[i].MacAddress[2],
           ED[i].MacAddress[3], ED[i].MacAddress[4], ED[i].MacAddress[5],
           ED[i].MacAddress[6], ED[i].MacAddress[7]);
