...
#define EVENT_LIST E_UPGRADE_START, E_UPGRADE_FILE_DOWNLOAD, E_UPGRADE_FINISH

#define STATE_LIST S_IDLE, S_UPGRADE_DOWNLOAD, S_UPGRADE_FINISH

#define ACTION_LIST                                                            \
  A_UPGRADE_SEND_CLEAR, a_send_clear, A_UPGRADE_SEND_IMAGE, a_send_image,      \
      A_UPGRADE_SEND_ACTIVE, a_send_active

typedef void (*upgrade_fsm_action_t)(void *);

typedef enum { DECLARE_ENUM(EVENT_LIST) } upgrade_event_ids_t;

typedef enum { DECLARE_ENUM(STATE_LIST) } upgrade_state_ids_t;

typedef enum { DECLARE_ENUM_PAIR(ACTION_LIST) } upgrade_action_ids_t;
...
static const fsm_transition_t upgrade_fsm_transition_table[] = {
    FSM_STATE(S_IDLE),
    FSM_TRANSITION(E_UPGRADE_START, FSM_NO_GUARD, A_UPGRADE_SEND_CLEAR,
                   S_UPGRADE_DOWNLOAD),

    FSM_STATE(S_UPGRADE_DOWNLOAD),
    FSM_TRANSITION(E_UPGRADE_FILE_DOWNLOAD, FSM_NO_GUARD, A_UPGRADE_SEND_IMAGE,
                   S_UPGRADE_DOWNLOAD),
    FSM_TRANSITION(E_UPGRADE_FINISH, FSM_NO_GUARD, A_UPGRADE_SEND_ACTIVE,
                   S_IDLE),
};
...
#if FSM_DEBUG
static const char *m_action_lookup_table[] = {DECLARE_STRING_PAIR(ACTION_LIST)};

static const char *m_guard_lookup_table[] = {DECLARE_STRING_PAIR(GUARD_LIST)};

static const char *m_event_lookup_table[] = {DECLARE_STRING(EVENT_LIST)};

static const char *m_state_lookup_table[] = {DECLARE_STRING(STATE_LIST)};
#endif /* FSM_DEBUG */

static const fsm_const_descriptor_t upgrade_fsm_descriptor = {
    .transition_table = upgrade_fsm_transition_table,
    .transitions_count = sizeof(upgrade_fsm_transition_table) /
                         sizeof(upgrade_fsm_transition_table[0]),
...
static void a_send_clear(void *p_data) {
  unsigned char fw_clear_request_cmd[] = {0xFF, 0xFC, 0xFC, 0xFF, 0x07,
                                          0x00, 0x00, 0x00, 0xE1, 0x00,
                                          0x00, 0x00, 0x08};

  libezmesh_write_ep(endpoint, &fw_clear_request_cmd[0],
                     sizeof(fw_clear_request_cmd), 0);
}

static void a_send_image(void *p_data) {
  unsigned char ota_download_request_cmd[512] = {
      0xFF, 0xFC, 0xFC, 0xFF, 157, 0x01, 0x00, 0x00, 0xE1, // Command id
  ...
  if (fp) {
    if (current_pkt == total_pkt - 1) {
      fread(&ota_download_request_cmd[DW_REQ_PKT_OFFSET],
            (file_size % g_pkt_size), 1, fp);
      ...
      ota_download_request_cmd[DW_REQ_FIXED_LEN + pkt_len - 1] =
          _gateway_checksum_calc(&ota_download_request_cmd[4],
                                 30 + (file_size % g_pkt_size));
    ...
    total_pkt =
        file_size / g_pkt_size + ((file_size % g_pkt_size != 0) ? 1 : 0);
    ...
    memcpy(&ota_download_request_cmd[28], (unsigned char *)&current_pkt, 4);
    ota_download_request_cmd[DW_REQ_FIXED_LEN + g_pkt_size - 1] =
        _gateway_checksum_calc(&ota_download_request_cmd[4], 30 + g_pkt_size);
...
static void a_send_active(void *p_data) {
  unsigned char fw_active_cmd[] = {0xFF, 0xFC, 0xFC, 0xFF, 0x07, 0x02, 0x00,
                                   0x00, 0xE1, 0x00, 0x00, 0x00, 0x07};
...
uint32_t startup(void) {
  ...
  ret = libezmesh_open_ep(lib_handle, &endpoint, EP_USER_ID_0,
...
static uint32_t crc32checksum(uint8_t *data, uint32_t data_len) {
    uint16_t k;
    uint8_t *buf = data;
    uint32_t chkSum = ~0;
...
int main(int argc, char *argv[]) {
  ...
  crc32 = crc32checksum(image_data, file_size);
  printf("file CRC: %04X\n", crc32);
  free(image_data);
...
void *rx_handler(void *ptr) {
  ...
  while (run) {
    ...
    size = libezmesh_read_ep(endpoint, &data_from_ezmesh[0],
                             FROM_EZMESH_BUF_SIZE, EP_READ_FLAG_NONE);
    if (size > 0) {
      pt_pd = (gateway_cmd_pd *)&data_from_ezmesh[5];
      cmd_index = pt_pd->command_id;

      if (cmd_index == 0xE1008000) {
        nanosleep((const struct timespec[]){{0, 50000}}, NULL);
        fsm_event_post(&upgrade_fsm, E_UPGRADE_FILE_DOWNLOAD, NULL);
      } else if (cmd_index == 0xE1008001) {
        timeout = 1;

        rsp_cnt = (unsigned char)pt_pd->parameter[0] |
                  ((unsigned char)pt_pd->parameter[1] << 8) |
                  ((unsigned char)pt_pd->parameter[2] << 16) |
                  ((unsigned char)pt_pd->parameter[3] << 24);
