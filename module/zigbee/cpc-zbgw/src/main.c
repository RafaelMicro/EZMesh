...
#define CPC_RESET_SLEEP_NS  10000000L
...
int main(int argc, char *argv[])
{
	...
	res = libcpc_init(&lib_handle, cpc_instance, reset_cb);

	res = libcpc_open_ep(lib_handle,
						&endpoint,
						CPC_EP_ZIGBEE,
						1);
...
{
	ssize_t ret;
	ret = libcpc_write_ep(endpoint, pdata, len, CPC_EP_WRITE_FLAG_NONE);
...
void *rx_handler(void *ptr)
{
	...
	while (run) 
	{
		...
		if (size > 0) 
		{
			Queue_TX_Write(data_from_cpc, size);
