...
static bool core_pull_frame_from_hal(frame_t **frame_buf, size_t *frame_buf_len);
static status_t core_push_data_to_server(uint8_t ep_id, const void *data, size_t data_len);
static void core_fetch_secondary_debug_counters(hal_epoll_event_data_t *event_data);
...
static inline bool __hdlc_is_poll_final(uint8_t control) { return (control & (1 << HDLC_CONTROL_P_F_POS))? true: false; }
...
static status_t state_passer(ez_err_t val) { return (val == NO_ERROR)? STATUS_OK : STATUS_FAIL; }
...
ep_state_t core_endpoint_state(uint8_t state)
{
    ...
    default:
        log_crash("A new state (%d) has been added to the Secondary that has no equivalent on the daemon.", state);
    }
    return state;
}
...
void core_init(int hal_fd, int hal_notify_fd)
{
    ...
    if (config.stats_interval > 0)
    {
        ...
        event_epoll = (hal_epoll_event_data_t *)calloc(1, sizeof(hal_epoll_event_data_t));
...
void core_process_transmit_queue(void)
{
    do 
    {
        if (transmit_queue == NULL && pending_on_security_ready_queue == NULL) break;   
        if (!core_proc_tx_data()) break;
    } while(1);
}
...
void core_set_endpoint_state(uint8_t ep_id, ep_state_t state)
{
    ...
    {
        log_info("Changing ep#%d state from %s to %s", ep_id, core_stringify_state(core_endpoints[ep_id].state), core_stringify_state(state));
                                                ...
                                                status_t status)
{
    ...
    if (status != STATUS_OK && status != STATUS_IN_PROGRESS) log_crash("Exit"); 
    if (property_id == PROP_LAST_STATUS)  log_error("Secondary does not handle the DEBUG_COUNTERS property, please update secondary or disable print-stats"); 
...
static void core_fetch_secondary_debug_counters(hal_epoll_event_data_t *event_data)
{
    ...
    sys_param_get(cpcd_update_secondary_debug_counter, PROP_EZMESHD_DEBUG_COUNTERS, 0, 0, false);
}
...
static void core_proc_rx_hal_notification(hal_epoll_event_data_t *event_data)
{
    ...
    ret = recv(hal_sock_notify_fd, &tx_complete_timestamp, sizeof(tx_complete_timestamp), MSG_DONTWAIT);
    ...
    case HDLC_FRAME_TYPE_IFRAME:
        if (frame->endpoint->state != ENDPOINT_STATE_OPEN) core_clear_tx_queue(&core_endpoints[frame->endpoint->id].retry_queue, -1);
        else
        {
            if (frame->endpoint->packet_retry_count == 0u)  frame->endpoint->last_iframe_sent_timestamp = tx_complete_timestamp; 
            if (frame->endpoint->retry_queue != NULL && frame->acked == false)  start_retry_timer(frame->endpoint, tx_complete_timestamp);
            if (frame->acked)  proc_ack(frame->endpoint, frame->pending_ack); 
        }
        break;

    case HDLC_FRAME_TYPE_UFRAME:
    case HDLC_FRAME_TYPE_SFRAME:
        if (frame->data_length != 0) free((void *)frame->data);
...
static void core_proc_rx_hal(hal_epoll_event_data_t *event_data)
{
    ...
    if (core_pull_frame_from_hal(&rx_frame, &frame_size) == false) return;
    ...
    data_length = rx_frame->header[HDLC_LENGTH_POS] | (rx_frame->header[HDLC_LENGTH_POS + 1] << 8);
    ...
    {
        if (type != HDLC_FRAME_TYPE_SFRAME)  transmit_reject(NULL, address, 0, HDLC_REJECT_UNREACHABLE_ENDPOINT);
        free(rx_frame);
        return;
    }

    if (type == HDLC_FRAME_TYPE_IFRAME || type == HDLC_FRAME_TYPE_SFRAME)  proc_ack(endpoint, ack);
    ...
    default:{
        ...
        log_info("[Core] EP #%u: rxd S-frame dropped", endpoint->id);
        break;}
    }
...
bool core_endpoint_is_closing(uint8_t ep_id) { return (core_endpoints[ep_id].state == ENDPOINT_STATE_CLOSING); }
...
void core_process_endpoint_change(uint8_t endpoint_number, ep_state_t ep_state)
{
    ...
    if (core_endpoints[endpoint_number].state != ENDPOINT_STATE_OPEN) core_open_endpoint(endpoint_number, 0, 1);
    return;
}

bool core_ep_is_busy(uint8_t ep_id) { return (core_endpoints[ep_id].holding_list != NULL); }
...
static void core_proc_rx_iframe(frame_t *rx_frame)
{
    ...
    log_info("[Core] EP #%u: rxd I-frame", endpoint->id);

    if (endpoint->id != 0 && (endpoint->state != ENDPOINT_STATE_OPEN || EP_list_empty(endpoint->id)))
    {
        transmit_reject(endpoint, address, 0, HDLC_REJECT_UNREACHABLE_ENDPOINT);
        return;
    }

    payload_length = rx_frame->header[HDLC_LENGTH_POS] | (rx_frame->header[HDLC_LENGTH_POS + 1] << 8);
    CHECK_ERROR(payload_length < HDLC_FCS_SIZE);
    payload_length = (uint16_t)(payload_length - HDLC_FCS_SIZE);
    fcs = rx_frame->payload[payload_length] | (rx_frame->payload[payload_length + 1] << 8);
    ...
    if (!core_check_crc_sw(rx_frame->payload, payload_length, fcs))
    {
        ...
        // log_info_INVALID_PAYLOAD_CHECKSUM();
        return;
    }
    ...
    if (seq == endpoint->ack)
    {
        if (control & (1 << HDLC_CONTROL_P_F_POS))
        {
            ...
            endpoint->poll_final.on_final(endpoint->id, (void *)HDLC_FRAME_TYPE_IFRAME, rx_frame->payload, payload_length);
        } 
        else
        {
            if (endpoint->id == EP_SYSTEM)
            {
                if (endpoint->on_iframe_data_reception != NULL)  endpoint->on_iframe_data_reception(endpoint->id, rx_frame->payload, payload_length);
            } 
            else
            {
                status_t status = core_push_data_to_server(endpoint->id, rx_frame->payload, payload_length);
    ...
    {
        log_info("EP #%u: rxd duplicate I-frame", endpoint->id);
        send_ack(endpoint);
    } 
    else
    {
...
static void core_proc_rx_sframe(frame_t *rx_frame)
{
    ...
    log_info("[Core] EP #%u: rxd S-frame", endpoint->id);

    switch (sframe_function)
    {
    case HDLC_ACK_SFRAME_FUNCTION:{
        log_info("[Core] EP #%u: rxd S-frame processed", endpoint->id);
        break;}

    case HDLC_REJECT_SFRAME_FUNCTION:{
        log_info("[Core] EP #%u: rxd S-frame rejected", endpoint->id);
        ...
        case HDLC_REJECT_SEQUENCE_MISMATCH:
            ...
            log_info("[Core] EP #%u: rxd reject seq mismatch", endpoint->id);
            log_warn("Sequence mismatch on endpoint #%d", endpoint->id);
            break;

        case HDLC_REJECT_CHECKSUM_MISMATCH:
            if (endpoint->retry_queue != NULL) retry_frame(endpoint);
            log_info("[Core] EP #%u: rxd reject checksum mismatch", endpoint->id);
            log_warn("Remote received a packet with an invalid checksum");
            break;

        case HDLC_REJECT_OUT_OF_MEMORY:
            log_info("[Core] EP #%u: rxd reject out of memory", endpoint->id);
            break;

        case HDLC_REJECT_UNREACHABLE_ENDPOINT:
            fatal_error = true;
            new_state = ENDPOINT_STATE_ERROR_DEST_UNREACH;
            log_info("[Core] EP #%u: rxd reject destination unreachable", endpoint->id);
        ...
        default:
            fatal_error = true;
            new_state = ENDPOINT_STATE_ERROR_FAULT;
            log_info("[Core] EP #%u: rxd reject fault", endpoint->id);
...
static void core_proc_rx_uframe(frame_t *rx_frame)
{
    ...
    address = rx_frame->header[HDLC_ADDRESS_POS]; 
    endpoint = find_endpoint(address);
    log_info("EP #%u: rxd U-frame", endpoint->id);

    control = rx_frame->header[HDLC_CONTROL_POS]; 
    type = (control >> HDLC_CONTROL_UFRAME_TYPE_POS) & HDLC_CONTROL_UFRAME_TYPE_MASK;

    payload_length = rx_frame->header[HDLC_LENGTH_POS] | (rx_frame->header[HDLC_LENGTH_POS + 1] << 8);
    payload_length = (payload_length < 2) ? 0 : (uint16_t)(payload_length - HDLC_FCS_SIZE);
    ...
    if (payload_length > 0)
    {
        ...
        {
            // log_info_INVALID_PAYLOAD_CHECKSUM();
            log_info("[Core] EP #%d: U-frame dropped : Bad payload checksum", ((endpoint == NULL) ? -1 : (signed)endpoint->id));
            return;
        }
    }
    if (!(endpoint->flags & OPEN_EP_FLAG_UFRAME_ENABLE))
    {
        log_info("[Core] EP #%d: U-frame dropped : U-Frame not enabled on endoint", ((endpoint == NULL) ? -1 : (signed)endpoint->id));
        return;
    }
    ...
    {
        log_info("[Core] EP #%d: U-frame dropped : Information U-Frame not enabled on endpoint", ((endpoint == NULL) ? -1 : (signed)endpoint->id));
        return;
    }
    ...
    case HDLC_CONTROL_UFRAME_TYPE_INFORMATION:{
        if (endpoint->on_uframe_data_reception != NULL) endpoint->on_uframe_data_reception(endpoint->id, rx_frame->payload, payload_length);
        break;}

    case HDLC_CONTROL_UFRAME_TYPE_POLL_FINAL:
        if (endpoint->id != EP_SYSTEM) log_error("Received an unnumbered final frame but it was not addressed to the system enpoint");
        else if (endpoint->poll_final.on_final != NULL) endpoint->poll_final.on_final(endpoint->id, (void *)HDLC_FRAME_TYPE_UFRAME, rx_frame->payload, payload_length);
        else log_crash("Exit");
        break;
    ...
    default:
        log_info("[Core] EP #%d: U-frame dropped : U-Frame not enabled on endpoint", ((endpoint == NULL) ? -1 : (signed)endpoint->id));
        return;
    }

    log_info("[Core] EP #%u: U-frame processed", endpoint->id);
}
...
void core_write(uint8_t endpoint_number, const void *message, size_t message_len, uint8_t flags)
{
    ...
    if ((flags & FLAG_UFRAME_INFORMATION) || (flags & FLAG_UFRAME_RESET_COMMAND) || (flags & FLAG_UFRAME_POLL))
    {
        ...
        iframe = false;
        if (flags & FLAG_UFRAME_INFORMATION)type = HDLC_CONTROL_UFRAME_TYPE_INFORMATION;
        else if (flags & FLAG_UFRAME_RESET_COMMAND) type = HDLC_CONTROL_UFRAME_TYPE_RESET_SEQ;
        else if ((flags & FLAG_UFRAME_POLL)) type = HDLC_CONTROL_UFRAME_TYPE_POLL_FINAL;
    }
    else CHECK_ERROR(endpoint->flags & OPEN_EP_FLAG_IFRAME_DISABLE); 
    ...
    transmit_queue_item = (transmit_queue_item_t *)calloc(1, sizeof(transmit_queue_item_t));
    ...
    else if (endpoint->current_tx_window_space <= 0) list_push_back(&endpoint->holding_list, &transmit_queue_item->node); 
    else
    {
...
void core_open_endpoint(uint8_t endpoint_number, uint8_t flags, uint8_t tx_win_size)
{
    endpoint_t *ep = NULL;
...
void core_set_endpoint_in_error(uint8_t endpoint_number, ep_state_t new_state)
{
    ...
    {
        log_warn("System endpoint in error, new state: %s. Restarting it.", core_stringify_state(new_state));
        sys_sequence_reset();
    } 
    else
    {
        log_warn("Setting ep#%d in error, new state: %s", endpoint_number, core_stringify_state(new_state));
...
static void core_clear_tx_queue(list_node_t **head, int endpoint_id)
{
    ...
    if (endpoint_id < 0) filter_with_endpoint_id = false;
    else
    {
    ...
    while (current_node)
    {
            ...
            || (filter_with_endpoint_id && item->handle->address == ep_id))
        {
            if (item->handle->pending_tx_complete == false)
            {
                ...
                if (item->handle->data_length != 0) free((void *)item->handle->data);
                free(item->handle);
...
status_t core_close_endpoint(uint8_t endpoint_number, bool notify_secondary, bool force_close)
{
    ...
    log_info("Closing endpoint #%d", endpoint_number);
    stop_retry_timer(ep);
    ...
    if (notify_secondary)
    {
        ...
        sys_param_set(disconnect_notification_callback, 5, /* 5 retries */ 100000, /* 100ms between retries*/ 
                             EP_ID_TO_PROPERTY_STATE(ep->id), &ep->state, sizeof(ep_state_t), false);
    }
...
static void proc_ack(endpoint_t *endpoint, uint8_t ack)
{
    ...
    if (endpoint->retry_queue == NULL) return;
    ...
    else if (ack > ack_range_max && ack < ack_range_min) return;
    ...
    log_info("%d Received ack %d seq number %d", endpoint->id, ack, seq_number);
    ...
    for (uint8_t i = 0; i < frames_count_ack; i++)
    {
        ...
        if (endpoint->id == EP_SYSTEM && __hdlc_is_poll_final(control)) sys_poll_ack(frame->data);
        ...
        if (endpoint->retry_queue == NULL) break;
    }
    ...
    log_info("[Core] EP #%u: rxd ack %u", endpoint->id, ack);
}
...
static void send_ack(endpoint_t *endpoint)
{
    ...
    log_info("Endpoint #%d sent ACK: %d", endpoint->id, endpoint->ack);
    core_process_transmit_queue();
    log_info("[Core] EP #%u: txd ack", endpoint->id);
}
...
static void retry_frame(endpoint_t *endpoint)
{
    ...
    // CHECK_ERROR(__hdlc_get_frame_type(item->handle->control) != HDLC_FRAME_TYPE_IFRAME);
    uint8_t type = 0;
    ...
    log_info("[Core] EP #%u: re-txd data frame", endpoint->id); 
    return;
}
...
static void transmit_reject(endpoint_t *endpoint, uint8_t address, uint8_t ack, reject_reason_t reason)
{
    ...
    control = HDLC_FRAME_TYPE_SFRAME << HDLC_CONTROL_FRAME_TYPE_POS;
    control |= (uint8_t)(HDLC_REJECT_SFRAME_FUNCTION << HDLC_CONTROL_SFRAME_FUNCTION_ID_POS);
    control |= ack;
    handle->control = control;
    ...
    *((uint8_t *)handle->data) = (uint8_t)reason;
    ...
    if (endpoint != NULL)
    {
        ...
        case HDLC_REJECT_CHECKSUM_MISMATCH:{
            ...
            log_warn("Host received a packet with an invalid checksum on ep %d", endpoint->id);
            break;}
        ...
        case HDLC_REJECT_UNREACHABLE_ENDPOINT:{
            log_error("[Core] EP #%d: txd reject destination unreachable", (endpoint == NULL) ? -1 : (signed)endpoint->id);
            break;}
    ...
    else
    {
        ...
        {
        case HDLC_REJECT_UNREACHABLE_ENDPOINT:{
            // log_info_TXD_REJECT_DESTINATION_UNREACHABLE();
            break;}
...
static bool core_proc_tx_data(void)
{
    ...
    {
        log_info("Sending packet that were hold back because security was not ready");
    ...
    else
    {
        ...
        {
            log_info("transmit_queue is empty and cpcd is not ready yet to process hold back packets");
            return false;
        }
    ...
    if (frame_type == HDLC_FRAME_TYPE_IFRAME) __hdlc_set_ctrl_ack(&frame->control, frame->endpoint->ack);
    else if (frame_type == HDLC_FRAME_TYPE_UFRAME) CHECK_ERROR(frame->endpoint->id != EP_SYSTEM);

    // log_info("[Core] frame address: %02x", frame->address);
    hdlc_create_header(frame->hdlc_header, frame->address, total_length, frame->control);
    ...
    if (encrypted_data_length != 0) memcpy(&frame_buffer->payload[encrypted_data_length], frame->fcs, sizeof(frame->fcs));
    ...
    free(frame_buffer);
    if (frame->data != encrypted_payload) free((void *)encrypted_payload);
    log_info("[Core] EP #%d: frame transmit submitted", (frame->endpoint == NULL) ? -1 : (signed)frame->endpoint->id);
...
static void retry_timeout(endpoint_t *endpoint)
{
    ...
    else
    {
        ...
        if (endpoint->retry_timeout_ms > MAX_RE_TRANSMIT_TIMEOUT_MS) endpoint->retry_timeout_ms = MAX_RE_TRANSMIT_TIMEOUT_MS;
        log_info("New RTO calculated on ep %d, after re_transmit timeout: %ldms", endpoint->id, endpoint->retry_timeout_ms);
...
static void stop_retry_timer(endpoint_t *endpoint)
{
    ...
    CHECK_ERROR(timerfd_settime(fd_timer_data->file_descriptor, 0, &cancel_time, NULL) < 0);
}

static double diff_timespec_ms(const struct timespec *final, const struct timespec *initial) { return (double)((final->tv_sec - initial->tv_sec) * 1000) + (double)(final->tv_nsec - initial->tv_nsec) / 1000000.0; }
...
static void start_retry_timer(endpoint_t *endpoint, struct timespec offset)
{
    ...
    if (offset_in_ms < 0) offset_in_ms = 0;
    if (endpoint->state != ENDPOINT_STATE_OPEN) return;
    CHECK_ERROR(fd_timer_data == NULL);
    CHECK_ERROR(fd_timer_data->file_descriptor < 0);

    timeout_time.it_interval.tv_sec = 0;
    timeout_time.it_interval.tv_nsec = 0;
    timeout_time.it_value.tv_sec = ((offset_in_ms + endpoint->retry_timeout_ms) / 1000);
    timeout_time.it_value.tv_nsec = (((offset_in_ms + endpoint->retry_timeout_ms) % 1000) * 1000000);

    CHECK_ERROR(timerfd_settime(fd_timer_data->file_descriptor, 0, &timeout_time, NULL) < 0);
}
...
static void core_push_frame_to_hal(const void *frame, size_t frame_len)
{
    ...
    log_info_hexdump("[Core] Push frame to hal (host uart tx): ", frame, frame_len);
    ret = send(hal_sock_fd, frame, frame_len, 0);
    CHECK_ERROR(ret < 0);
    CHECK_ERROR((size_t)ret != frame_len);
    // log_info_TXD_TRANSMIT_COMPLETED();
}
...
static bool core_pull_frame_from_hal(frame_t **frame_buf, size_t *frame_buf_len)
{
    ...
    if (retval == 0)
    {
        log_info("Driver closed the data socket");
    ...
    *frame_buf_len = (size_t)datagram_length;
    ...
    log_info_hexdump("[Core] Pull frame from hal (host uart rx): ", *frame_buf, *frame_buf_len);

    return true;
}

static status_t core_push_data_to_server(uint8_t ep_id, const void *data, size_t data_len) { return state_passer(EP_push_data(ep_id, (uint8_t *)data, data_len)); }

uint16_t core_get_crc_sw(const void *buf, uint16_t length)
{
    uint16_t crc = 0;
    for (uint16_t i = 0; i < length; i++) crc = core_compute_crc16((uint8_t)((uint8_t *)buf)[i], crc);
    return crc;
}

bool core_check_crc_sw(const void *buf, uint16_t length, uint16_t expected_crc) { return (core_get_crc_sw(buf, length) == expected_crc); }
...
void hdlc_create_header(uint8_t *hdr,
                        uint8_t address,
                        uint16_t length,
                        uint8_t control)
{
