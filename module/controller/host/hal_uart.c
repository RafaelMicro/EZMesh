...
static uartConfig_t uart_config[] = {
    {9600, B9600},       {115200, B115200},   {500000, B500000},
...
static bool __hdlc_header_validate(uint8_t *hdr) {
  uint16_t hcs = 0;
  if (hdr[HDLC_FLAG_POS] != HDLC_FLAG_VAL)
    return false;
...
static bool __sync_header(uint8_t *buffer, size_t *pos) {
  ...
  /* If we think of a header like a sliding window of width
   * SLI_CPC_HDLC_HEADER_RAW_SIZE, then we can slide it 'num_header_combination'
   * times over the data. */
  ...
  for (i = 0; i != num_header_combination; i++) {
    if (__hdlc_header_validate(&buffer[i])) {
      if (i == 0) {
        // The start of the buffer is aligned with a good header, don't do
        // anything
      } else {
        /* We had 'i' number of bad bytes until we struck a good header, move
         * back the data to the beginning of the buffer */
  ...
  //     }
  // }
  // memmove(&buffer[0], &buffer[num_header_combination], HDLC_HEADER_RAW_SIZE -
  // 1); *pos = HDLC_HEADER_RAW_SIZE - 1;
...
static bool __push_valid_hdlc_frame(uint8_t *buffer, size_t *pos) {
  ...
  payload_len =
      (uint16_t)(buffer[HDLC_LENGTH_POS] | (buffer[HDLC_LENGTH_POS + 1] << 8));
  ...
  if (frame_size > *pos)
    return false;
  log_debug_hexdump("[uart rx]", buffer, frame_size);
...
static void __hal_uart_proc(void) {
  ...
  if (t.tv_nsec + ns > UNIT_1G)
    t.tv_sec += (t.tv_nsec + ns) / UNIT_1G;
  t.tv_nsec += ns;
  t.tv_nsec %= UNIT_1G;
...
pthread_t hal_uart_init(int *fd_to_cpcd, int *fd_notify_cpcd,
  ...
  log_info("[HAL] dev: %s, baudrate: %d, hardflow: %d", device, baudrate,
           hardflow);
  ...
  *fd_notify_cpcd = fd_sockets_notify[1];
  ...
  CHECK_ERROR(
      pthread_create(&tx_drv_thread, NULL, __hal_uart_transmit_thd, NULL) != 0);
  CHECK_ERROR(
      pthread_create(&rx_drv_thread, NULL, __hal_uart_receive_thd, NULL) != 0);
  CHECK_ERROR(
      pthread_create(&cleanup_thread, NULL, __hal_uart_cleanup_thd, NULL) != 0);
...
int hal_uart_open(const char *device, unsigned int baudrate, bool hardflow) {
  ...
  if (sym_baudrate < 0)
    log_info("wrong baudrate: %d", baudrate);
