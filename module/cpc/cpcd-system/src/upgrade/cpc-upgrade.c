...
static void a_send_clear(void *p_data)
{
    ...
    libcpc_write_ep(endpoint, &fw_clear_request_cmd[0], sizeof(fw_clear_request_cmd), 0);
}
...
static void a_send_image(void *p_data)
{
    ...
    if (fp)
    {
        if (current_pkt == total_pkt - 1)
        {
            ...
            libcpc_write_ep(endpoint, &ota_download_request_cmd[0], pkt_len, 0);
...
static void a_send_active(void *p_data)
{
    ...
    libcpc_write_ep(endpoint, &fw_active_cmd[0], sizeof(fw_active_cmd), 0);
}
